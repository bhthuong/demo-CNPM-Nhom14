...
cate.post("/insertcate",(req,res)=>{
    ...
    cate.save(function(err,data){
        if(err){
            res.redirect("/icate",{ message : "Thêm mới không thành công" })
        } else{
            cates.find().then(function (data) {
                res.render("admin/list_cate", { item: data, message: "Thêm mới thành công", });
            });
        }
    })
})
...
cate.get("/admin/edit_cate/:id",(req,res)=>{
    cates.findById(req.params.id,function(err,data){
        if(!err) {
            res.render("admin/edit_cate", {item : data})
        }
    })
})

cate.post("/updatecate",(req,res)=>{
    cates.updateOne({namecate : req.body.namecate}, function(err) {
        if(err){
            res.redirect("/admin/edit_cate",{message :"Cập nhật không thành công"})
        } else{
            cates.find().then(function (data) {
                res.render("admin/list_cate", { item: data,message: "Cập nhật thành công", });
            });
        }
    })
})
    ...
    if (req.session.loggin) {
        cates.deleteOne({ _id: req.params.id }, function (err) {
            ...
            res.redirect("/admin/listcate");
                ...
                cates.find().then(function (data) {
                    res.render("admin/list_cate", { item: data, message: "Xóa thành công", });
                });
            }
        });
    } else {
