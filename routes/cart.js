...
cart.get("/admin/insert_order",(req,res)=>{
  res.render("admin/insert_order");
})

cart.post("/insertorder",(req,res)=>{
  var order = new Cart({
    status: "Đã xử lý",
    name: req.body.name,
    address: req.body.address,
  ...
  order.save(function(err,data){
      if(err){
          res.redirect("admin/list-order",{ message : "Thêm mới không thành công" })
      } else{
          Cart.find().then(function (data) {
              res.render("admin/list-order", { data: data, message: "Thêm mới thành công", });
          });
      }
  })
})
